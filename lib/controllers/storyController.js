var storyController = function(Story, Event, StoryProvider) {
  ...
  function refreshStories(callback) {
    ...
    StoryProvider.getStories(function(storiesArray){
      console.log("Refreshing stories");
    
      // clear the old stories here
      Story.remove().exec();
      
      // storiesArray is an array where each element is a list of
      // stories from one story provider. 
      var stories = [];
      storiesArray.forEach(function(storiesArrayElement) {
          stories = stories.concat(storiesArrayElement);
      }, this);
